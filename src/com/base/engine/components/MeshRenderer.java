...
public class MeshRenderer extends GameComponent
{
	...
	private static final float SIZE_WATER = 1600;//16/1/20
	//private static final int VERTEX_COUNT = 128;//12/1/20 - terraine test
	private static final float MAX_HEIGHT = 40;
	private static final float MAX_PIXEL_COLOUR = 256 * 256 * 256;
	private static final float MAX_PIXEL_COLOUR_WATER = 512 * 512 * 512;
...
public MeshRenderer(Material material, String heightMap, boolean isWater) {
		...
		for(int i=0;i<VERTEX_COUNT;i++){
			for(int j=0;j<VERTEX_COUNT;j++){
				vertices[vertexPointer*3] = (float)j/((float)VERTEX_COUNT - 1) * SIZE_WATER;  //0
				
				float height = getHeight(j, i, image, true);
				//Random waterHeight = new Random();
				//heights[j][i] = waterHeight.nextInt(6);//height;
				
				vertices[vertexPointer*3+1] = height;//waterHeight.nextInt(6);//									    //1
				vertices[vertexPointer*3+2] = (float)i/((float)VERTEX_COUNT - 1) * SIZE_WATER;//2
				normals[vertexPointer*3] = 0;											//0
				normals[vertexPointer*3+1] = 1;											//1
				normals[vertexPointer*3+2] = 0;											//2
				textureCoords[vertexPointer*2] = (float)j/((float)VERTEX_COUNT - 1);    //0
...
public void waterHeightUpdate(String heightMap) {
	...
	for(int i=0;i<VERTEX_COUNT;i++){
		for(int j=0;j<VERTEX_COUNT;j++){
			...
			vertices[vertexPointer*3+1] = waterHeight.nextInt(6);//height;									    //1
			vertices[vertexPointer*3+2] = (float)i/((float)VERTEX_COUNT - 1) * SIZE;//2
			normals[vertexPointer*3] = 0;											//0
			normals[vertexPointer*3+1] = 1;											//1
			normals[vertexPointer*3+2] = 0;											//2
	...
	private float getHeight(int x, int y, BufferedImage image, boolean isWater) {
		if(x<0 || x>=image.getHeight() || y<0 || y>=image.getHeight()) {
			return 0;
		}
		float height = image.getRGB(x, y);
